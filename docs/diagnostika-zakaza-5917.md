...
Мета пустая → в Яндекс уходит заявка без пункта назначения → API отвечает
`Not found station by station_id`, и вариантов доставки нет. Это подтверждено на самом заказе
#5917: диагностика вернула ровно эту ошибку при пустом ПВЗ получателя и корректном пункте
отгрузки магазина (`278bfc4d-f56c-401a-b6b7-d6d6e784bf3e`).
